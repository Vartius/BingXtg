...
def setup_database(db_file):
    ...
    try:
        ...
        for index in indexes:
            ...
            if len(index_info) == 1 and index_info[0][2] == 'message_id' and index[2] == 1:  # unique index
        ...
        if not has_unique_constraint:
            try:
                cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_labeled_message_id_unique ON labeled(message_id)")
                logger.info("✅ Added unique constraint on labeled.message_id")
            except sqlite3.IntegrityError as e:
                logger.error(f"❌ Cannot add unique constraint due to existing duplicates: {e}")
                ...
                if duplicates:
                    logger.warning(f"⚠️  Found {len(duplicates)} message_ids with duplicates:")
                    ...
                    logger.info("💡 Consider cleaning up duplicates before running the labeling process")
                else:
...
def get_already_labeled_ids(cursor):
    ...
    try:
        ...
        logger.info(f"📊 Found [bold yellow]{len(labeled_ids)}[/bold yellow] previously labeled messages")
        return labeled_ids
    except Exception as e:
...
def get_unlabeled_messages(cursor):
    ...
    try:
        ...
        logger.info(f"📝 Found [bold cyan]{len(messages)}[/bold cyan] unlabeled messages to process")
        return messages
    except Exception as e:
...
def save_to_database(conn, cursor, message_id, channel_id, message_text, data):
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM labeled WHERE message_id = ?", (message_id,))
        ...
        if is_signal:
            ...
            dialog_message(f"🎯 SIGNAL FOUND: {pair} {direction} (ID: {message_id})", "signal")
        ...
        if "UNIQUE constraint failed" in str(e):
            ...
            dialog_message(f"Integrity error saving data for message {message_id}: {str(e)[:100]}", "error")
            return False
    except Exception as e:
        dialog_message(f"Failed to save data for message {message_id}: {str(e)[:100]}", "error")
        return False
    
def configure_apis():
    """Configures all AI APIs with the provided keys."""
    global github_client, groq_client, together_client, anthropic_client, cohere_client, perplexity_client
    try:
        ...
        if not github_token:
            ...
            raise ValueError("GITHUB_TOKEN environment variable not set. Please add GITHUB_TOKEN=your_github_token_here to your .env file")
        ...
        if groq_api_key:
            ...
            dialog_message("GROQ_API_KEY not set - Groq models will be skipped", "warning")
        ...
        if together_api_key:
            ...
            dialog_message("TOGETHER_API_KEY not set - Together AI models will be skipped", "warning")
        ...
        if anthropic_api_key:
            ...
            dialog_message("ANTHROPIC_API_KEY not set - Anthropic models will be skipped", "warning")
        ...
        if cohere_api_key:
            ...
            dialog_message("COHERE_API_KEY not set - Cohere models will be skipped", "warning")
        ...
        if perplexity_api_key:
            ...
            dialog_message("PERPLEXITY_API_KEY not set - Perplexity AI models will be skipped", "warning")
...
def get_current_model():
    ...
    if not AVAILABLE_MODEL_CONFIGS:
        raise ValueError("AVAILABLE_MODEL_CONFIGS is empty. Make sure configure_apis() and get_available_models() are called first.")
...
def switch_to_next_model():
    ...
    try:
        ...
        dialog_message(f"Failed to create instance for {new_model_config['name']}: {str(e)[:100]}", "error")
        current_model_instance = None
    
    update_model_display_info(new_model_config, "Active")
    
    # Show model switch in dialog
    dialog_message(f"Model switch: {old_model_config['name']} → {new_model_config['name']} (Rate limit)", "model")
...
def create_model_instance(model_config):
    ...
    global groq_client, together_client, anthropic_client, cohere_client, perplexity_client
    ...
    if model_config["type"] == "gemini":
        ...
        if groq_client is None:
            logger.warning(f"⚠️  Groq client not configured, skipping model: {model_config['name']}")
            raise ValueError("Groq client not configured")
        return groq_client
    elif model_config["type"] == "together":
        if together_client is None:
            logger.warning(f"⚠️  Together AI client not configured, skipping model: {model_config['name']}")
            raise ValueError("Together AI client not configured")
        return together_client
    elif model_config["type"] == "anthropic":
        if anthropic_client is None:
            logger.warning(f"⚠️  Anthropic client not configured, skipping model: {model_config['name']}")
            raise ValueError("Anthropic client not configured")
        return anthropic_client
    elif model_config["type"] == "cohere":
        if cohere_client is None:
            logger.warning(f"⚠️  Cohere client not configured, skipping model: {model_config['name']}")
            raise ValueError("Cohere client not configured")
        return cohere_client
    elif model_config["type"] == "perplexity":
        if perplexity_client is None:
            logger.warning(f"⚠️  Perplexity AI client not configured, skipping model: {model_config['name']}")
...
def get_api_response(model_config, model_instance, prompt):
    ...
    try:
        # Add debugging info
        logger.debug(f"API call: {model_config['type']} model {model_config['name']}, instance: {type(model_instance)}")
        ...
        if model_config["type"] == "gemini":
            ...
            return response.content[0].text.strip().replace("```json", "").replace("```", "").strip()
        ...
        logger.error(f"API call failed for {model_config['type']} model {model_config['name']}: {e}")
        raise
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if current_model is None:
                dialog_message(f"Model instance is None, recreating {current_config['name']}", "warning")
                current_model = create_model_instance(current_config)
                current_model_instance = current_model
            
            dialog_message(f"Sending request to {current_config['name']} (attempt {attempt + 1}/{MAX_RETRIES})", "info")
            response_text = get_api_response(current_config, current_model, prompt)
            dialog_message(f"Received response from {current_config['name']}", "success")
            ...
            if "429" in error_str or "quota" in error_str.lower() or "rate limit" in error_str.lower():
                dialog_message(f"Rate limit hit with {current_config['name']}", "warning")
                ...
                if switch_to_next_model():
                    ...
                    try:
                        ...
                        dialog_message(f"Switched to model: {new_model_config['name']} ({new_model_config['type']})", "model")
                        processing_stats["current_model"] = new_model_config["name"]
                        
                        response_text = get_api_response(current_config, current_model, prompt)
                        dialog_message(f"Success with new model: {new_model_config['name']}", "success")
                        return response_text
                        
                    except Exception as model_switch_error:
                        dialog_message(f"Error with new model {new_model_config['name']}: {str(model_switch_error)[:100]}", "error")
                        if attempt < MAX_RETRIES - 1:
                            delay = exponential_backoff_delay(attempt)
                            dialog_message(f"Waiting {delay:.1f}s before retry {attempt + 1}/{MAX_RETRIES}", "warning")
                            time.sleep(delay) # Use synchronous sleep
                            continue
                else:
                    if attempt < MAX_RETRIES - 1:
                        delay = exponential_backoff_delay(attempt)
                        dialog_message(f"All models exhausted. Waiting {delay:.1f}s before retry", "warning")
                        time.sleep(delay) # Use synchronous sleep
                        continue
                    else:
...
def get_api_response_wrapper(model_config, model_instance, prompt):
    ...
    response = get_response_with_retry(model_config, model_instance, prompt, is_batch=False)
...
def process_batch(model_config, model_instance, messages_batch):
    ...
    global current_model_index, current_model_instance
    dialog_message(f"Processing batch of {len(messages_batch)} messages with {model_config['name']}", "info")
    batch_prompt = create_batch_prompt(messages_batch)
    json_response = get_response_with_retry(model_config, model_instance, batch_prompt, is_batch=True)
    updated_model_config = get_current_model()
    ...
    try:
        ...
        results_map = {str(result['message_id']): result for result in batch_results if isinstance(result, dict) and 'message_id' in result}
        matched_results = []
        for msg_id, channel_id, message_text in messages_batch:
...
def cleanup_duplicate_labeled_entries(conn, cursor):
    ...
    try:
        ...
        for message_id, count in duplicates:
            ...
            logger.debug(f"📝 Removed {removed} duplicate entries for message_id {message_id}")
        
        conn.commit()
        logger.info(f"✅ Cleaned up {total_removed} duplicate entries from labeled table")
        return True
...
def get_available_models():
    ...
    logger.info(f"✅ Available models: {len(available_models)} out of {len(MODEL_CONFIGS)} total")
...
def should_process_message(message_text):
    ...
    definitely_not_signals = [
        ...
        'good morning everyone', 'good night everyone', 'how is everyone',
        'thank you so much', 'thanks everyone', 'welcome to our',
        # Admin messages
        'message deleted', 'user banned', 'group rules', 'pinned message'
    ]
...
class SignalLabelerApp(App):
        ...
        with Container(id="app-grid"):
            ...
            with Vertical(id="middle-column"):
                ...
                with Horizontal(id="button-container"):
                    ...
                    yield Button("Clear Selection", variant="primary", id="clear-selection")
    ...
    def watch_processed_count(self, new_count: int) -> None:
        ...
        if self.total_messages > 0:
            self.query_one(ProgressBar).progress = (new_count / self.total_messages) * 100
    ...
    def watch_current_message_text(self, new_text: str) -> None:
        if not self.selected_message:
            ...
            content = f"[bold cyan]ID:[/bold cyan] {self.current_message_id}\n\n{new_text}"
    ...
    def add_log_message(self, message: str, msg_type: str = "info") -> None:
        ...
        icons = {"info": "ℹ️", "success": "✅", "warning": "⚠️", "error": "❌", "signal": "🚀", "model": "🔄"}
    ...
    def update_extracted_labels(self, data: dict) -> None:
        ...
        if data.get('is_signal', False):
            pair = data.get('pair', 'N/A')
            direction = data.get('direction', 'N/A')
            direction_color = "green" if direction == "LONG" else "red" if direction == "SHORT" else "white"
    ...
    def add_message_to_history(self, message_id: str, message_text: str, extracted_data: dict | None = None) -> None:
    ...
    def add_message_to_history_panel(self, message_entry: dict) -> None:
        ...
        text_preview = text_preview[:40] + "..." if len(text_preview) > 40 else text_preview
        ...
        button = Button(button_text, variant=button_variant, id=f"msg_{message_id}", classes="message-button")
    ...
    def on_mount(self) -> None:
        ...
        self.query_one("#history-container").mount(Static("[dim]History will appear here.[/dim]", id="history-placeholder"))
    ...
    def on_clear_selection_pressed(self) -> None:
        ...
        try:
            selected_button = self.query_one(f"#msg_{self.selected_message['message_id']}", Button)
        ...
        self.add_log_message("Cleared message selection, returning to live processing view", "info")
    ...
    def on_message_button_pressed(self, event: Button.Pressed) -> None:
        ...
        try:
            ...
            if self.selected_message:
                 prev_button = self.query_one(f"#msg_{self.selected_message['message_id']}", Button)
                 is_signal = self.selected_message.get('is_signal', False)
        ...
        selected_msg_data = next((msg for msg in self.processed_messages_history if msg['message_id'] == message_id), None)
        ...
        if selected_msg_data:
            ...
            self.add_log_message(f"Viewing message ID {message_id} from history", "info")
        else:
            self.add_log_message(f"Could not find data for message ID {message_id}", "error")
    ...
    async def process_messages_worker(self) -> None:
        ...
        try:
            ...
            await asyncio.to_thread(self._blocking_database_and_api_work, ui_update_callback)
        except Exception as e:
    ...
    def handle_worker_update(self, event_type: str, data: dict):
        ...
        if event_type == "log":
            ...
            if data["extracted_data"]:
                self.extracted_labels_content = self.format_extracted_labels(data["extracted_data"])
            ...
            self.add_message_to_history(data["message_id"], data["message_text"], data["extracted_data"])
    ...
    def _blocking_database_and_api_work(self, callback):
        ...
        try:
            ...
            if not AVAILABLE_MODEL_CONFIGS:
                callback("log", {"message": "No AI models available! Check .env file.", "type": "error"})
                return
            ...
            callback("log", {"message": "Fetching unlabeled messages...", "type": "info"})
            all_messages = get_unlabeled_messages(cursor)
            if not all_messages:
                callback("log", {"message": "No new messages to label.", "type": "success"})
                return

            # Randomize message processing order
            all_messages = list(all_messages)  # Convert to list if it's not already
            random.shuffle(all_messages)
            callback("log", {"message": f"Shuffled {len(all_messages)} messages for random processing order", "type": "info"})

            valid_messages = []
            skipped_count = 0
            for msg_id, channel_id, msg_text in all_messages:
                if not msg_text or not msg_text.strip() or not should_process_message(msg_text):
                    non_signal_data = {"is_signal": False}
                    save_to_database(conn, cursor, msg_id, channel_id, msg_text, non_signal_data)
            ...
            callback("log", {"message": f"Skipped {skipped_count} non-signals. Processing {len(valid_messages)} via API.", "type": "info"})
            
            # Shuffle valid messages again for maximum randomness
            if valid_messages:
                random.shuffle(valid_messages)
                callback("log", {"message": f"Shuffled {len(valid_messages)} valid messages for batch processing", "type": "info"})
            ...
            for i in range(0, len(valid_messages), BATCH_SIZE):
                ...
                callback("log", {"message": f"Processing batch {i//BATCH_SIZE + 1}/{total_batches}...", "type": "info"})

                batch_results, new_config, new_instance = process_batch(model_config, model_instance, batch)
                ...
                for msg_id, channel_id, message_text, data in batch_results:
                    save_to_database(conn, cursor, msg_id, channel_id, message_text, data)
                    callback("progress", {
                        "message_id": str(msg_id),
                        "message_text": message_text,
                        "extracted_data": data
            ...
            callback("log", {"message": f"A critical error occurred in worker: {e}", "type": "error"})
        finally:
            if conn:
                conn.close()
                callback("log", {"message": "Database connection closed.", "type": "info"})
